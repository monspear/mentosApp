          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if(snapshot.exists){ // 이미 존재하는 id 인 경우
                  flag = 5;
                   ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                ...
                if (flag == 0) {
                  try {
                    ...
                    final docUser = FirebaseFirestore.instance
                        .collection('users') 
                        .doc(checkInfo[2]); 
                        // firebase database에 'users'라는 컬렉션에 사용자가 입력한 아이디로 작은 문서를 만듬

                    debugPrint("2번");
                    var year = birthdayController.text.toString().substring(0,4);
                    var month = birthdayController.text.toString().substring(4,6);
                    var day = birthdayController.text.toString().substring(6,8); // 생일을 저장하기 위함
                    ...
                    final userInfoJson = User( // 위에 언급한 작은 문서에 저장할 사용자의 개인정보를 다음 형식으로 지정
                      ...
                      userbirthday: DateTime(int.parse(year), int.parse(month), int.parse(day)),
                    );

                    await docUser.set(userInfoJson.toJson()); // 사용자의 개인정보를 문서로 만들어 저장

                    /*
                await FirebaseAuth.instance.createUserWithEmailAndPassword(// 파이어베이스어스 회원가입하는  부분
                  email: checkInfo[2],
                  password: checkInfo[3],
                  );
                  */
